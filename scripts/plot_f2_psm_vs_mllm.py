#!/usr/bin/env python3
"""F2 paper figure: PSM vs any-of-K coverage retrieval ablation across the
v1 street-scale corpus.

Per session, plots two grouped bars:
  - Any-of-K coverage at K=8: a hit if ANY of the 8 uniformly-sampled frames'
    timestamps fall in a GT interval (exemplar_hit_rate_at_5 in the gemini.json).
    This is an upper bound on Gemini's single-frame pick (see §4.5), NOT the pick
    rate. Read from captures/mllm_baseline/<sess>_gemini.json
...
def _psm_clipl_r12_hit5(captures_dir: Path, sequence: str) -> float | None:
    ...
    rx = re.compile(rf"^h3_res_12_{re.escape(sequence)}_clipL_s(?P<seed>\d+)\.json$")
    rates = []
...
def _render_svg(rows: list[dict]) -> str:
    ...
    lines.append(
        ...
        f'font-weight="bold">PSM vs any-of-K coverage (K=8 uniform frames)</text>'
    )
    ...
    lgx = ML + PW + 30
    lgy = MT + 30
    lines.append(f'<text x="{lgx}" y="{lgy - 12}" font-weight="bold">Method</text>')
    for i, (color, label, desc) in enumerate(
        ...
        lines.append(f'<text x="{lgx + 20}" y="{y + 2}">{label}</text>')
        lines.append(
    ...
    for r in rows:
        ...
        if r["psm"] is not None and r["mllm"] is not None:
            if r["mllm"] > 0:
                ...
                ratio = "∞"
        print(
            f"{r['corpus']:10s}  {r['sequence'][:35]:35s}  {psm:>7s}  {mllm:>7s}  {ratio:>5s}"
        )
    print()
