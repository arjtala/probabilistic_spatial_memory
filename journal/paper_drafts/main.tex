...
for embedding-similarity precision.

At the bounded deployment point ($R{=}128$, \texttt{cap}=$K$),
\psm{} reaches 21 of 25 brute-force-groundable queries on a
187-narration Nymeria proxy session; with reservoir eviction removed
($R{=}1024$), the same endpoint recovers all 25---a
substrate-correctness sanity check, not a performance claim.
