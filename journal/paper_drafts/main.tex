...
On 102.9\,h of metropolitan driving (Honda HDD~\cite{ramanishka2018hdd}) with
genuine revisits, \psm{} exercises the multi-session regime its design targets:
state that grows \emph{sublinearly} with observation count---$O(\text{area
explored})$, not $O(\text{time})$---at $1.3$--$7.2\times$ below a dense bank;
unique-event cardinality that accrues across 219 days in a $1\,\mathrm{KiB}$ HLL
sketch while decaying between visits; and label-free cross-session place
recognition (0.853 AUC vs.\ a 0.498 shuffled null).
