...
session vs.\ brute-force CLIP's 25/187 with session-length-invariant
per-cell state; removing reservoir eviction ($R{=}1024$, $8\times$ the
per-cell budget) recovers the identical 25/187, a substrate-correctness check.
...
$K{=}8$ sampling yields Hit@5 $=0\%$. On 102.9\,h of metropolitan
driving (Honda HDD~\cite{ramanishka2018hdd}) with genuine revisits, \psm{}
exercises its target multi-session regime: \emph{sublinear} state
($O(\text{area})$, not $O(\text{time})$; $1.3$--$7.2\times$ below a dense
bank), cross-session cardinality accruing across 219 days in a
$1\,\mathrm{KiB}$ sketch, and label-free place recognition (0.853 AUC vs.\ a
0.498 null). The C engine ($\sim$700 LoC) and Python harness are released
under MIT.
