...
in-reservoir exemplars. On the 25 narrations where the ground-truth
interval is uniquely groundable, \psm{} at \texttt{cap}=$K$ recovers
all 25 (\emph{conditional accuracy $=100\%$}).
...
the spatial-temporal prefilter that frontier MLLMs currently
lack: at matched candidate budget ($K{=}8$), \psm{}'s cell-targeted
retrieval reaches every session that vanilla Gemini~3.1~Pro
uniform-sampling misses, including 4 sessions where the uniform
$K{=}8$ frames structurally never land in a $\sim$3\,s GT interval.
The C engine ($\sim$700 LoC) and
