...
for embedding-similarity precision: at the bounded deployment point
($R{=}128$, \texttt{cap}=$K$), it reaches 21/187 Hit@5 on the
Nymeria proxy session vs.\ brute-force CLIP's 25/187; with reservoir
eviction removed ($R{=}1024$), the same endpoint recovers the identical
25/187 hit set, a substrate-correctness sanity check rather than a
performance claim.
...
query-matched H3 cells rather than uniform time slices, it provides
the spatial-temporal prefilter that frontier MLLMs
lack: on four of these sessions, uniform $K{=}8$ sampling yields
Hit\,@\,5 $= 0\%$---the frontier MLLM never sees a ground-truth
frame, regardless of its reasoning capability.
\psm{}'s cell-targeted retrieval removes this structural blind spot
in those sessions. The C engine ($\sim$700 LoC) and
