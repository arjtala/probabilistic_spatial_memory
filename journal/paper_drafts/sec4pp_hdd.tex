...
102.9\,h with valid GPS). A vehicle sweeps far more area per minute than a
walker and, over 132 drives across 8 months, genuinely re-drives the same
streets---the multi-session revisit structure a single-pass egocentric
corpus lacks. HDD ships no look-back-QA labels, so these results are
...
days (pre-registered acceptance bar: $\geq30\%$), across 5{,}163 revisited
cells whose first-to-last visit spans a \textbf{median of 106 days} (max
221). The coverage is arterial, not a depot artifact---excluding the 20
most-driven cells leaves it at 46.5\%.
...
smaller than a dense per-frame bank at every ingest rate, and the gap
widens as the reservoir discards more redundant dwell/revisit frames:
$1.3\times$/$2.3\times$/$4.3\times$/$7.2\times$ at 1/5/15/30\,fps (capping
26/57/77/86\% of frames). State thus grows \emph{sublinearly in
observation count}---$O(\text{distinct cells})$, not $O(\text{frames})$.
This is \emph{not} an observed plateau: cumulative area keeps growing (only
20\% of cells are seen by the halfway mark), so the saving comes from
per-cell capping and is ingest-rate dependent---the empirical form of
``budgeted by area, not sequence length.''
...
observations over 219 days}, while its 60-slot ring-buffer windowed
estimate decays between visits and jumps on each revisit---decay while
maintaining unique-event cardinality across revisits. A Python
reimplementation matches the C-engine sketch to $2.4\times10^{-5}$ median
relative error.
...
other cells. Over 5{,}222 revisited cells (2{,}000 stratified queries) the
cross-session AUC is \textbf{0.853}, vs.\ a shuffled-cell null of $0.498$
and a same-drive upper bound of $0.977$---strong cross-session place
recognition from CLIP cosine over the persistent per-cell exemplars alone.
The same-cell-vs-all-others split blends geographic proximity with visual
