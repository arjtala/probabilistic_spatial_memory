...
$1.3\times$/$2.3\times$/$4.3\times$/$7.2\times$ at 1/5/15/30\,fps---state
grows \emph{sublinearly in observation count}, $O(\text{distinct cells})$
not $O(\text{frames})$. This is not a plateau (only 20\% of cells seen by
the halfway mark); the saving is per-cell capping---``budgeted by area,
not sequence length.''
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{figures/hdd_combined.pdf}
