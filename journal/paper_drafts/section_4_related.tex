...
Ego4D~\cite{grauman2022ego4d} established look-back QA at scale;
EgoSchema~\cite{mangalam2023egoschema} exposed the temporal-grounding collapse
our prefilter targets. The ``where did I leave my keys?'' task
...
SAVEMem~\cite{wu2026savemem} all bound memory in \emph{token} space (KV-cache
compression, dense-to-sparse merging, recurrent state). \psm{} instead bounds
memory in \emph{geographic} space (H3 cells) and runs upstream: it decides
\emph{which frames} the MLLM sees, whereas these compress what it sees---
...
axis. \psm{} adds a ``near here'' H3 $k$-ring restriction and fully streaming
ingestion with ring-buffer-aged per-cell state. The H3 grid~\cite{brodsky2018h3}
replaces R-tree subdivisions~\cite{guttman1984rtrees} with equidistant neighbors
and stable cell ids; the primitives (HyperLogLog~\cite{flajolet2007hyperloglog},
reservoir sampling~\cite{vitter1985reservoir}) are standard---the contribution
is their composition under the \caponek{} knob.
