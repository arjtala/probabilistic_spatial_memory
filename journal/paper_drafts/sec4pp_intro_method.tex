...
linearly with recording duration, making continuous multi-session memory
prohibitively expensive on-device. Improving the long-context capacity of
the models themselves~\cite{arora2024linear,chen2024vljepa} does not
address the underlying systems bottleneck: how to structure a dynamic
spatial representation that supports test-time memory updating without
unbounded state explosion.

Three properties define a viable substrate for multi-session persistent
spatial memory. \emph{Memory-budgeted}: state must be bounded
independently of sequence length, scaling instead by physical area
explored. \emph{Dynamic}: the index must support test-time evolution,
letting old information decay while maintaining unique-event cardinality
across revisits. \emph{Streaming}: observations must update the memory
in-place and online. We present \textbf{Probabilistic Spatial Memory}
(\psm{}), a substrate meeting all three, and demonstrate the
multi-session regime it targets on 102.9\,h of metropolitan driving with
genuine same-place revisits---the setting a single-pass egocentric corpus
cannot exercise.
...
bounded-memory top-$K$ similarity search at any time. Three layered
probabilistic data structures over an H3~\cite{brodsky2018h3} hexagonal
tessellation enforce the memory bound.
...
For look-back QA the ranked candidates feed a frontier-MLLM reranker; the
full pipeline and architecture diagram are in the extended paper.
