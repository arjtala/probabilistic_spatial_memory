...
Three extensions are immediate: (a)~an \emph{on-device rerank} swapping
Gemini for a small VLM (Phi-3.5-Vision, SmolVLM) to test the
\caponek{}+reranker composition on wearable compute; (b)~\emph{counting
QA} surfacing the validated HLL cardinalities (\S\ref{sec:results-hll},
median relative error $1.0\%$) to a question class beyond
moment-grounding (``how many distinct people did I see in the
kitchen?''); and (c)~\emph{federated memory}, merging two devices'
per-cell state via HLL union and reservoir sampling without sharing raw
frames---H3 cell ids are global.
