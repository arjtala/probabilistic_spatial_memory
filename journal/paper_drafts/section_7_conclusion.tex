...
\psm's per-cell-capped state stays $1.3$--$7.2\times$ smaller than a
dense embedding bank while a $1\,\mathrm{KiB}$ sketch accrues
unique-event cardinality across 219 days and label-free
cross-session place recognition reaches $0.853$ AUC---the
multi-session persistence the single-pass corpus cannot exercise
(the synthetic memory crossover at $N{\approx}1500$ frames is in
supp.\ Fig.~\ref{fig:f6-memory}).
