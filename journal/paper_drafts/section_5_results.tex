...
On the short Nymeria sessions ($N\in[748,1325]$, ${\sim}22$\,min) brute-force is still marginally cheaper---the synthetic crossover falls at $N\approx1500$ frames, past which the gap reaches ${\sim}10$--$20\times$---so the bounded-memory payoff is demonstrated on the multi-hour HDD corpus (\S\ref{sec:results-hdd}), where \psm{} is smaller at every ingest rate.
...
On a Galaxy S22 (Exynos~2200, Cortex-X2) the same engine benchmark ingests at $0.86\,\mu$s/frame and answers location/semantic queries in $104\,\mu$s\,/\,$1.16$--$1.75$\,ms (\caponek{}$=1$/$K$) at $20.2$\,MiB RSS---a $2.6$--$3.5\times$ slowdown vs.\ the identical bench on host ($0.33\,\mu$s, $30\,\mu$s, $0.37$--$0.55$\,ms), so the microsecond constants carry to wearable-class ARM at sub-2\,ms, tens-of-MiB cost (engine core only; CLIP encoding and I/O excluded, \S\ref{sec:limitations}).
The \caponek{} latency ordering flips with density: on the dense Nymeria bench \caponek{}$=1$ is slower (its $K$ distinct cells each force a separate time-envelope merge, the dominant cost), while on the sparse S22 workload \caponek{}$=K$ is slower (its larger candidate sort dominates); cosine scoring is identical across settings.
...
Nymeria micro-eval validates per-session accuracy: median $1.0\%$ relative error
across cells, comfortably below HLL's theoretical relative standard error
($1.04/\sqrt{2^{10}}\approx3.25\%$ at $p{=}10$, a standard error, not a hard
bound); a Python reimplementation matches the C sketch to $2.4\times10^{-5}$
median error, confirming this is the engine's behavior, not an analysis artifact.
This accuracy supports counting queries (``how many distinct people did I interact
with in the kitchen?'') natively on the substrate, without a downstream MLLM
recounting frames. Is that memory retrievable across sessions? Label-free,
