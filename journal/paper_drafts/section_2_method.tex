...
$r\in\{10,11,12,13\}$ in supp.\ \S\ref{sec:supp-ablations}). H3 cell IDs are
...
(${\sim}0.44$\,MiB/cell at $R{=}128$, CLIP-L's $d{=}768$). Brute-force grows
$\Theta(N)$; \psm{} grows $\Theta(R\cdot|\mathrm{cells}|)$---bounded by the area
...
values trade off smoothly (\S\ref{sec:results-main}). The novelty is composing
this per-partition limit---standard in query planning---with H3 tessellation,
HLL sketches, and reservoir sampling under streaming constraints. Ingest is
$O(1)$ amortized; queries run any time after the first ingest (microsecond
host-CPU constants in supp.\ \S\ref{sec:supp-memory-latency}).
...
architectural commitment: one factory serves CLIP-L (430\,M, 768\,d), CLIP-bigG
(2.5\,B, 1280\,d), and SigLIP-2-large~\cite{tschannen2025siglip2}
(660\,M, 1024\,d); the C engine sees only a $D$-dim vector, and cross-encoder
generalization (\S\ref{sec:results-multi-corpus}) confirms the behavior is not a
CLIP-L artifact.
