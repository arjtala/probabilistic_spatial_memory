...
\coordinate (cellAnchor) at ($(hexcenter) + (13mm, 0mm)$);
% HLL ring (top): two faded (old) + three fresh
\node[hllold] (hll1) at ($(cellAnchor) + (-3mm, 2.8mm)$) {};
\node[hllold] (hll2) at ($(cellAnchor) + (-1mm, 2.8mm)$) {};
\node[hllbox] (hll3) at ($(cellAnchor) + ( 1mm, 2.8mm)$) {};
\node[hllbox] (hll4) at ($(cellAnchor) + ( 3mm, 2.8mm)$) {};
\node[font=\tiny, right=0.4mm of hll4, anchor=west]
     {HLL ring \textit{(decay)}};
% Reservoir (bottom)
\node[resvslot] (r1) at ($(cellAnchor) + (-3mm, 0mm)$) {};
\node[resvslot] (r2) at ($(cellAnchor) + (-1mm, 0mm)$) {};
\node[resvslot] (r3) at ($(cellAnchor) + ( 1mm, 0mm)$) {};
\node[resvslot] (r4) at ($(cellAnchor) + ( 3mm, 0mm)$) {};
\node[font=\tiny, right=0.4mm of r4, anchor=west] {$R$ exemplars};
\node[font=\tiny\itshape, below=1.4mm of r2, gray!60!black]
     {bounded / cell};
\draw[smallflow, dashed, blue!50!black]
     (hC.east) -- ($(hll1.west) + (-0.6mm, -1.4mm)$);

% ---- query ----
\node[box, right=15mm of r4, align=center, fill=orange!10,
...
\draw[flow, dashed, blue!50!black]
     ($(r4.east)+(0.5mm,0)$) -- ++(3mm,0) |- (topk.west);
