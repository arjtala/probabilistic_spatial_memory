...
- 2026-06-21 — **14-session vanilla-MLLM baseline closed; F2 spans the full corpus and PSM beats Gemini-as-retrieval 14/14 with median ~4×. Multi-corpus story landed across §1/§3/§4/§5/§6/§7.** The LookOut MLLM baseline loop that had bailed on a PNG-vs-JPEG glob mismatch (fix landed Jun 20 in commit `d47f9d4` — re-derive 1 fps subsample of `rgb_info.pkl` capture timestamps so kept PNGs align 1:1 with H5 ts, mirroring `aria_generate_questions.py`) ran clean across all 10 LookOut sessions. Combined with the prior 3 SLOPER4D + 1 Nymeria runs this fills out the full 14-session v1 corpus on a matched K=8 candidate budget (commit `663c6f1`). Headline read across sessions:

    | regime | sessions | description |
    |---|---|---|
    | vanilla MLLM = 0 % | **4/14** | Mainquad_jan10, Fostersquare1_jan16, Huang_Gates_jan10 (LookOut) + shelby_arroyo_act0 (Nymeria) — K=8 uniform frames structurally never overlap the GT interval window |
    | PSM beats MLLM by 2-10× | 8/14 | spatial discrimination narrows the 8 candidates from "the whole session" to "8 candidates near where the query is *likely* to be" |
    | Tie (13.8 %) | 1/14 | Hillsdale6_jan14, also PSM's weakest LookOut session under loose acceptance — visually-poor queries for both methods |
    | PSM > 0, MLLM ≈ PSM | 1/14 | filled in by the rerank story, not the vanilla baseline |

    F2 regenerated to span all 14 sessions (`scripts/plot_f2_psm_vs_mllm.py`, originally landed Jun 20 as `93b5daf`; the script's missing-data-aware glob auto-grew the figure to 14 bars on rerun). F3 already at 14 sessions × 3 encoders from Jun 20 (`2e38edc`); F6 honest crossover plot also from Jun 20 (`f995e80`). With F2/F3/F6 anchored on the captures, a four-commit propagation wave touched every section that referenced the old Nymeria-only framing:

    - `1573268` (§5): new `tab:multi-corpus-h3` + ~3 paragraphs of corpus/encoder description and three-pattern interpretation; `+cmark/xmark` macros for PASS/FAIL marks.
    - `663c6f1` (§5): new `sec:results-vanilla-mllm` "PSM is necessary, not optional" subsection + F2 + the temporal-coverage gap explanation (K=8 uniform frames over 100-640 s sessions cover ~4 % of any GT interval) + the seq009 K-sweep oracle-vs-discrimination decomposition (oracle 21.4 % at K=32, Gemini's actual pick rate 7.1 %).
    - `113e195` (§1/§5/§6/§7): §1 multi-session bullet extended; §5 memory/latency rewritten with the F6 crossover math (PSM ~13 MB constant vs brute-force ~3 MB at corpus scale, crossover at N≈4400 frames ≈ 73 min @ 1 fps, diverges to 3-6× at 4-8 h streams); new `sec:results-mllm-rerank` subsection + `tab:reranker` with the bimodal-Hit@1 finding promoted into the body; §6 limitations "One honest spatial-axis failure" paragraph naming BurlingameDT4_feb5; §7 conclusion rewritten around the multi-corpus / multi-encoder / vanilla-MLLM result.
    - `ab5c1ea` (abstract + §3 + §4): abstract replaces the Nymeria-only headline with the 14-session × 3-corpus × 3-encoder spatial-axis result and the matched-budget vanilla-MLLM claim; §4 gets vision-language-encoders paragraph (CLIP family / SigLIP 2 / Long-CLIP / MetaCLIP / EVA-CLIP / DINOv2 / V-JEPA 2 as natural swaps, weller2025limit as the upper bound) and an egocentric-outdoor-corpora paragraph anchoring SLOPER4D / Nymeria / LookOut against the Aria-MPS vs head-mounted-LiDAR sensor-stack split + licences.

    **Off-by-one catch (`9f48165`)**: abstract / §1 / §5.6 caption / §7 all said "five sessions land at MLLM Hit@5 = 0" — audit of `captures/mllm_baseline/*_gemini.json` shows exactly **four** (3 LookOut + Nymeria). Corrected; all other multi-corpus headline numbers (13/14 any-enc, 9/14 clipL, 10/14 bigG, 11/14 SigLIP, PSM beats MLLM 14/14 median ~4×) re-verified against the raw captures.

    Next: bootstrap-CI the headline mIoU comparison; submission housekeeping (F1 figure, reproduce script, citation pass, anonymization sweep).
- 2026-06-22 — **Submission freeze: bootstrap-CI surfaces a stale mIoU number; F1 + reproduce script + two new citations land; PAPER.md closed by this entry.** Two distinct landings on the day.

    **Bootstrap CI on the headline mIoU comparison (`41091a6` + `d504076`).** Re-evaluated `shelby_arroyo_act0` with a new `--diff` mode in `scripts/bootstrap_ci.py` — paired percentile bootstrap on per-question (a-b) with shared resample indices to preserve question pairing, joining records by `id` across `eval_lookback.py`'s `records` key and `eval_psm_mllm.py`'s `questions_out` key. Live capture said:

    | method | exemplar mIoU @ 5 | 95 % CI |
    |---|---|---|
    | PSM cap=1 + Gemini | **0.069** | [0.049, 0.092] |
    | brute-force CLIP-L | 0.074 | [0.050, 0.102] |
    | paired Δ (n=187, 10 k resamples, seed=42) | **-0.005** | **[-0.032, +0.021]** |

    CI on the difference comfortably brackets zero — the original "statistically indistinguishable" claim is justified — **but** the PSM number had drifted: paper said 0.073, live capture gives 0.069. Corrected the value everywhere it appeared (abstract-adjacent intro bullet, §3 architectural-decomposition paragraph, §5.2 headline table + commentary + caption, §7 conclusion) and upgraded each phrasing from "matches" to "is statistically indistinguishable... (Δ = -0.005, 95 % CI [-0.032, +0.021])". Hit@5 (8.0 %, 15/187) and brute-force mIoU (0.074) were already correct; only the PSM cap=1+Gemini cell needed updating. **The value of verifying numbers against live captures during freeze, not before**: a 0.004 drift on a single cell would have shipped as-is without the rerun. The CI machinery is now permanently part of `reproduce_paper.sh` so future drifts surface automatically.

    **Submission housekeeping wave (commits `feca0b3` + `5bfd40b` + `c5bcce3`).** (a) **F1 architecture figure** finally has a backing file: pure-TikZ single-column figure at `journal/paper_drafts/figures/f1_architecture.tex` showing streaming input → PSM ingest (H3 hex grid + per-cell ring of HLLs + reservoir of exemplars) → top-K retrieval with `per_cell_cap` gate → MLLM rerank flow. `\input` from §3 at the head of the Pipeline subsection; adds the `tikz` package and `positioning / arrows.meta / fit / calc / shapes.geometric / backgrounds` libs to `main.tex`. Resolves the previously dangling `fig:f1-architecture` label. (b) **`scripts/reproduce_paper.sh`** — one-shot driver that runs Table 2 H3 acceptance verdicts (14 sessions), F2 PSM-vs-vanilla-MLLM table, F2/F3/F6 figure regeneration, and the bootstrap CI on the headline Aria 4-session detail. Does **not** re-run encoder extraction or MLLM API calls (those need cluster GPU + `GEMINI_API_KEY`); header documents what is and isn't regenerated and points at `scripts/aria_acceptance.sh` / `scripts/sloper4d_acceptance.sh` for the full upstream pipeline. §7 conclusion footnote calls this out as the canonical entry point. (c) **Two arXiv-grounded citations** — `lee2025aria2` (Aria2 system architecture, arXiv 2512.16045) cited in §1 ¶1 as the Reality Labs system architecture that identifies a persistent spatial-temporal index as a first-class component (PSM is one concrete implementation), and `yang2026egocopilot` (Egocentric Co-Pilot, arXiv 2603.01104) cited in §4 ¶2 as a smart-glasses agent framework with temporal chain-of-thought + hierarchical context compression (PSM is the natural bounded-memory retrieval frontend such agents would query). (d) **Anonymization sweep across §1–§7 + bib confirms author / institute / email already double-blind; no real identifiers leak from the paper body.** No diff needed — landed as a verification-only pass folded into the citations commit.

    **Open items as of freeze**:

    - **F7 (qualitative top-5 candidates over a query)** — still not started. Currently the only figure in the §1 figure-pipeline table without a backing script. Decision needed before submission whether to ship without it (current §5 visual evidence is the F2/F3/F6 trio + per-session tables) or block on a one-evening screenshot pass through `psm-viz`.
    - **`requirements.txt` pin for the script-side artifact** — `extraction/pyproject.toml` is pinned; the script-runner side hasn't been audited. Trivial; do before tagging.
    - **`git tag eccv2026-submission`** — pending freeze commit.
    - **PAPER.md** — closed by this entry; further work moves to the in-progress task tracker and `journal/EXPERIMENTS.md`.
